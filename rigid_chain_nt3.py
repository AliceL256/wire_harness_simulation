...
class Chain:
    def __init__(self, viewer):
        ...
        for chain_idx in range(self.num_chains):
            builder.add_articulation(key=f"pendulum_{chain_idx}")

            # create links for this chain
            links = []
            ...
            for i, link in enumerate(links):
                if i == 0:
                    parent = -1
                    base_x = chain_idx * chain_spacing
                    parent_xform = wp.transform(p=wp.vec3(base_x, 0.0, 10.0), q=wp.quat_identity()) # initial position for this chain
